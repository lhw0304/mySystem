...
public interface MultiSelectDAO {
    ...
    @Select({"<script>",
            ...
            "</where>",
            "</script>"})
    public Integer countMultiKnowledge(@Param("userId") Integer userId);

    @Select({"<script>",
            "SELECT  (SELECT id FROM md_multi_select  WHERE knowledge = main.knowledge ORDER BY rand()  LIMIT 0,1) as id",
            "FROM md_multi_select main  ",
            "<where>",
            "<if test='userId != null'> user_id = #{userId} </if>",
            "</where>",
            "GROUP BY knowledge limit #{limit}",
            "</script>"})
    public List<Integer> getMultiSelectIdList(@Param("userId") Integer userId,
                                               @Param("limit") Integer limit);

    @Select({"<script>",
            "select * from (",
            "SELECT (SELECT id FROM md_multi_select  WHERE user_id = #{userId} and knowledge = main.knowledge " ,
            "ORDER BY rand()  LIMIT 0,1) as id",
            "FROM md_multi_select main GROUP BY knowledge union ",
            "(select id from md_multi_select where user_id = #{userId} order by rand() limit #{restCount}) )a limit #{multiCount}",
            "</script>"})
    public List<Integer> getMultiSelectIdList2(@Param("userId") Integer userId,
                                                @Param("restCount") Integer restCount,
                                                @Param("multiCount") Integer multiCount);

    @Select({
            "<script>",
            "select * from md_multi_select where id in (${stringMultiIds})",
            "</script>"
    })
    @Results({
            @Result(property = "id", column = "id"),
            ...
            @Result(property = "knowledge", column = "knowledge"),
            @Result(property = "ctime", column = "ctime")})
    public List<MultiSelect> getGroupList(@Param("stringMultiIds") String stringMultiIds);
