function shortUploadCtrl($http,$scope,$cookies,utilService) {
	$scope.userId = $cookies.get("userId");
	$scope.shortUpload = function() {
		var formData = new FormData();
		formData.append("content",$scope.content);
		formData.append("answer",$scope.answer);
		var req = {
			method: "POST",
			url : webUriPrefix + "/system/short/create",
			headers : {
...
function shortListCtrl($http, utilService, $scope, $cookies,SweetAlert,$compile,$state,$rootScope) {
	$scope.userId = $cookies.get("userId");
        ...
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    var req = {
                        method: "POST",
                        url: webUriPrefix + '/system/short/delete/' + id,
                    };
                    $http(req)
                        .success(function (response) {
                            if (response.message.code == 0) {
    ...
    $scope.update = function(id) {
    	$cookies.put("shortId", id);
    	$state.go("short.update")
    }
}
function shortUpdateCtrl($scope,$http,$cookies,utilService) {
	$scope.userId = $cookies.get("userId");
	$scope.shortId = $cookies.get("shortId");
	$scope.getShort = function() {
		var req = {
			method : "GET",
			url : webUriPrefix + "/system/short/fetch/" + $scope.shortId,
		}
		$http(req)
		.success(function(response){
			if (response.message.code == 0) {
				$scope.content = response.payload.shortAnswer.content;
				$scope.answer = response.payload.shortAnswer.answer;
			};
		})
	}
	$scope.getShort();
	$scope.updateShort = function() {
		...
		var req = {
			method: "POST",
			url : webUriPrefix + "/system/short/update/" + $scope.shortId,
			headers : {
				'Content-Type': undefined,
				'userId' : $scope.userId,
			},
...
angular.module('inspinia')
    .controller('shortUploadCtrl', shortUploadCtrl)
    .controller('shortUpdateCtrl', shortUpdateCtrl)
