function singleUploadCtrl($http,$scope,$cookies,utilService) {
	...
	$scope.singleUpload = function() {
		...
		var req = {
			method: "POST",
			url : webUriPrefix + "/system/single/create",
			headers : {
...
function singleListCtrl($http, utilService, $scope, $cookies,SweetAlert,$compile,$state,$rootScope){
	$scope.userId = $cookies.get("userId");
        ...
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    var req = {
                        method: "POST",
                        url: webUriPrefix + '/system/single/delete/' + id,
                    };
                    $http(req)
                        .success(function (response) {
                            if (response.message.code == 0) {
    ...
    $scope.update = function(id) {
    	$cookies.put("singleId", id);
    	$state.go("single.update")
    }
}
function singleUpdateCtrl($scope,$http,$cookies,utilService){
	$scope.userId = $cookies.get("userId");
	$scope.singleId = $cookies.get("singleId");
	$scope.getSingle = function() {
		var req = {
			method : "GET",
			url : webUriPrefix + "/system/single/fetch/" + $scope.singleId,
		}
		$http(req)
		.success(function(response){
			if (response.message.code == 0) {
				$scope.content = response.payload.singleSelect.content;
				$scope.answer = response.payload.singleSelect.answer;
				$scope.a = response.payload.singleSelect.a;
				$scope.b = response.payload.singleSelect.b;
				$scope.c = response.payload.singleSelect.c;
				$scope.d = response.payload.singleSelect.d;
			};
		})
	}
	$scope.getSingle();
	$scope.updateSingle = function() {
		...
		var req = {
			method: "POST",
			url : webUriPrefix + "/system/single/update/" + $scope.singleId,
			headers : {
				'Content-Type': undefined,
				'userId' : $scope.userId,
			},
...
angular.module('inspinia')
    .controller('singleUploadCtrl', singleUploadCtrl)
    .controller('singleUpdateCtrl', singleUpdateCtrl)
    .controller('singleListCtrl', singleListCtrl);